...
axes_kde[0].set_xlabel('Age (baseline) [years]')
...
axes_kde[1].set_xlabel('Age [years]')
axes_kde[1].set_ylabel('')
axes_kde[1].legend_.remove()

# Finalise styling
fig_kde = set_style_ax(fig_kde, axes_kde, both_axes=False)
fig_kde = set_size(fig_kde, 6, 2.5)
save_figure(fig_kde, 'figures/age_kde_plots')
plt.show()


# Target histogram and KDE plot
fig_target, axes_target = get_figures(n_rows=1, n_cols=2, figsize=(6, 6), sharex=False, sharey=False)

sns.histplot(data=data, x='duration', hue='sex', multiple='stack', bins=15, ax=axes_target[0])
axes_target[0].set_title('Longitudinal Data Target Histogram')
axes_target[0].set_xlabel('Age (baseline)')
axes_target[0].set_ylabel('Count')

sns.kdeplot(data=data, x='duration', hue='sex', ax=axes_target[1], fill=True, common_norm=False, linewidth=1.5)
axes_target[1].set_title('Longitudinal Data Target Density')
axes_target[1].set_xlabel('Inter-scan Interval [years]')
axes_target[1].set_ylabel('Density')
axes_target[1].legend_.remove()



# Finalise styling
fig_target = set_style_ax(fig_target, axes_target, both_axes=False)
fig_target = set_size(fig_target, 6, 2.5)
save_figure(fig_target, 'figures/target_histogram_kde.png')
plt.show()

print(f"Mean age in longitudinal dataset: {data['age'].mean():.2f} years")
print(f"Mean age in cross-sectional dataset: {data_cs['age'].mean():.2f} years")
print(f"Mean target in longitudinal dataset: {data['duration'].mean():.2f} years")
print(f"Min target {data['duration'].min():.2f} years, max target {data['duration'].max():.2f} years.")

print(f"r, p between age and target in longitudinal dataset: {pearsonr(data['age'], data['duration'])}")
