...
class loader3D(Dataset):
    ...
    def __init__(self, args, participant_df):
        ...
        for _, row in self.demo.iterrows():
            ...
            path = os.path.join('/mimer/NOBACKUP/groups/brainage/thesis_brainage/resized_images', f'{participant_id}_{session}_resized.pt')
            if not os.path.exists(path): #skip if no matching files are found
    ...
    def __getitem__(self, index):
        ...
        image_tensor = torch.load(path, weights_only=False)
