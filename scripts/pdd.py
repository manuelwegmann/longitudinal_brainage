...
def parse_args():
    ...
    parser.add_argument('--pdd', default='enter', type=str, help="type yes if pdd desired")
...
def train(opt, train_dataset, val_dataset):
    ...
    for epoch in range(opt.epoch, opt.max_epoch):
        ...
        for batch in dataloader_train:
            ...
            # Conditionally apply selective backprop
            if opt.pdd == 'yes':
                mask = abs_err > 0.8
                ...
                # Standard training on all samples
                datapoints_for_backprop += len(target)
                loss = criterion(output, target)
...
if __name__ == "__main__":
    ...
    val_fold.to_csv(os.path.join(output_dir, 'train_fold.csv'), index=False)
