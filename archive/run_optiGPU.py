...
def train(opt, train_dataset, val_dataset):
    ...
    dataloader_train = DataLoader(loader3D(opt, train_dataset), batch_size=opt.batchsize, shuffle=True, **kwargs)
    dataloader_val = DataLoader(loader3D(opt, val_dataset), batch_size=opt.batchsize, shuffle=False, **kwargs)
    ...
    for epoch in range(opt.epoch, opt.max_epoch):
        ...
        for batch in dataloader_train:
            ...
            print("Data device check: ", x1.device, x2.device, target.device, meta.device)
