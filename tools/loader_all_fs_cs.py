...
def build_participant_block(participant_id, sex, remove_fs = False, folder_path='/mimer/NOBACKUP/groups/brainage/data/oasis3', project_data_dir = '/mimer/NOBACKUP/groups/brainage/thesis_brainage/data'):
    """
...
class loader3D(Dataset):
    ...
    def __init__(self, args, participant_df):
        ...
        print("Remove based on FS: ", args.remove_fs)
        ...
        for _, row in df.iterrows():
            ...
            sex = str(row['sex'])
            block, clean = build_participant_block(participant_id, sex, remove_fs=args.remove_fs, folder_path=args.data_directory, project_data_dir=args.project_data_dir)
            if clean == 1:
                num_clean += 1
            else:
                num_unclean += 1
        ...
        print(f"Num of participants with age: ", num_clean)
        print(f"Num of participants without age: ", num_unclean)
