...
"""Target vs Residual"""
axes[0].scatter(targets_LILAC, res_LILAC, alpha=0.5, color='steelblue')
axes[0].set_xlabel('Target [years]')
axes[0].set_ylabel('Residual [years]')
axes[0].set_title('LILAC')

# Fit regression line
coeffs = np.polyfit(targets_LILAC, res_LILAC, deg=1)
...
"""
"""Target vs Residual"""
axes[1].scatter(targets_CS, res_CS, alpha=0.5, color='steelblue')
axes[1].set_xlabel('Target [years]')
axes[1].set_title('CS CNN')

# Fit regression line
coeffs = np.polyfit(targets_CS, res_CS, deg=1)
line = np.poly1d(coeffs)
...
"""Target vs Residual"""
axes[2].scatter(targets_LILACp, res_LILACp, alpha=0.5, color='steelblue')
axes[2].set_xlabel('Target [years]')
axes[2].set_title('LILAC+')

# Fit regression line
coeffs = np.polyfit(targets_LILACp, res_LILACp, deg=1)
line = np.poly1d(coeffs)
...
save_figure(fig, "figures/target_res.png")
